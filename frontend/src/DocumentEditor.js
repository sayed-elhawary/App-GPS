...
// ─── دالة لمعالجة وتحويل الأرقام في العقد النصية ───────────────────────────
const convertNumbersInNode = (node) => {
  if (node.nodeType === Node.TEXT_NODE) {
    if (node.textContent && /[0-9]/.test(node.textContent)) {
      const converted = convertToArabicNumbers(node.textContent);
    ...
    node.childNodes.forEach(convertNumbersInNode);
  }
};
...
// ─── حوار قائمة المستندات المحفوظة مع بحث ──────────────────────────────────────
function SavedDocsDialog({ onLoad, onClose, token }) {
  ...
  const handleDelete = async (id, e) => {
    ...
    try {
      setDeleting(id);
      const res = await fetch(`${process.env.REACT_APP_API_URL}/api/documents/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const highlightMatch = (text, query) => {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <mark style={{ background: '#a7f3d0', color: '#065f46', borderRadius: 3, padding: '0 2px' }}>
  ...
  return (
    <div className="tbl-overlay" onClick={onClose}>
      <div className="tbl-dialog" onClick={e => e.stopPropagation()} style={{ minWidth: 540, maxWidth: 640, maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
        <h3 className="tbl-title">📂 المستندات المحفوظة</h3>

        <div style={{ position: 'relative', marginBottom: 14 }}>
          <span style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', fontSize: 16, color: '#10b981', pointerEvents: 'none' }}>🔍</span>
          <input
            type="text"
          ...
          {searchQuery && (
            <button
              ...
              style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 16, lineHeight: 1, padding: 0 }}
              title="مسح البحث"
            >✕</button>
          )}
        </div>
        ...
        {!loading && !error && searchQuery.trim() && (
          <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 8, textAlign: 'right', direction: 'rtl' }}>
            {filteredDocs.length === 0
              ? 'لا توجد نتائج مطابقة'
              : `${filteredDocs.length} مستند${filteredDocs.length !== 1 ? 'ات' : ''} مطابق`}
          </div>
        )}
        ...
        <div style={{ overflowY: 'auto', flex: 1, paddingRight: 2 }}>
          ...
          {!loading && !error && filteredDocs.length === 0 && (
            <div style={{ textAlign: 'center', padding: '30px', color: '#94a3b8', fontSize: 14 }}>
              {searchQuery.trim() ? '🔍 لا توجد مستندات تطابق البحث' : 'لا توجد مستندات محفوظة بعد'}
            </div>
          )}
          {!loading && filteredDocs.map(doc => (
            <div key={doc._id} onClick={() => onLoad(doc)}
              ...
              <div style={{ flex: 1, overflow: 'hidden' }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: '#065f46', marginBottom: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  📄 {highlightMatch(doc.title || 'بدون عنوان', searchQuery)}
                </div>
        ...
        <div className="tbl-actions" style={{ marginTop: 16 }}>
          <button className="tbl-cancel" onClick={onClose}>إغلاق</button>
        </div>
      </div>
    </div>
  );
...
// ─── حوار خيارات الطباعة الكامل (طباعة مباشر + PDF) ─────────────────────────
function PrintOptionsDialog({ onClose, onPrint, onDirectPrint }) {
  ...
  const pageSizes = ['A3', 'A4', 'A5', 'B4', 'B5', 'Letter', 'Legal', 'Tabloid'];

  const marginPresets = {
    normal: { top: 1.5, bottom: 1.5, left: 1.5, right: 1.5 },
    narrow: { top: 0.8, bottom: 0.8, left: 0.8, right: 0.8 },
    wide: { top: 2.5, bottom: 2.5, left: 3, right: 3 },
  };

  const applyMarginPreset = (preset) => {
    setMargins(preset);
    if (preset !== 'custom') {
      const m = marginPresets[preset];
      setMarginTop(m.top); setMarginBottom(m.bottom);
      setMarginLeft(m.left); setMarginRight(m.right);
    }
  };

  const printSettings = { orientation, pageSize, margins, marginTop, marginBottom, marginLeft, marginRight, showHeader, showFooter, showSignature, colorMode, fontSize, lineHeight, repeatHeader };
  ...
  const fieldLabel = { fontSize: 12, color: '#374151', fontWeight: 700, marginBottom: 6, display: 'block', direction: 'rtl' };
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, direction: 'rtl' }}>
        ...
        <button style={toggleStyle(value)} onClick={() => onChange(!value)}>
          <div style={{ position: 'absolute', top: 3, left: value ? 3 : 'auto', right: value ? 'auto' : 3, width: 18, height: 18, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,.2)', transition: 'all .2s' }} />
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="tbl-overlay" onClick={onClose}>
      <div className="tbl-dialog" onClick={e => e.stopPropagation()} style={{ minWidth: 480, maxWidth: 540, maxHeight: '90vh', display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '20px 24px 0', borderBottom: '1px solid #e5e7eb' }}>
          <h3 style={{ fontSize: 17, fontWeight: 900, color: '#065f46', marginBottom: 14, textAlign: 'center' }}>
            🖨️ إعدادات الطباعة والتصدير
          </h3>
          <div style={{ display: 'flex', borderBottom: '1px solid #e5e7eb', marginBottom: 0, direction: 'rtl' }}>
            <button style={tabStyle('page')} onClick={() => setActiveTab('page')}>📄 الصفحة</button>
            <button style={tabStyle('content')} onClick={() => setActiveTab('content')}>📝 المحتوى</button>
            <button style={tabStyle('advanced')} onClick={() => setActiveTab('advanced')}>⚙️ متقدم</button>
          </div>
        </div>

        <div style={{ overflowY: 'auto', flex: 1, padding: '18px 24px', direction: 'rtl' }}>
          {activeTab === 'page' && (
            <>
              <div style={{ marginBottom: 18 }}>
                <label style={fieldLabel}>📐 حجم الورقة</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {pageSizes.map(size => (
                    <button key={size} onClick={() => setPageSize(size)} style={{
                      padding: '6px 14px', border: pageSize === size ? '2px solid #10b981' : '1.5px solid #d1d5db',
                      borderRadius: 8, background: pageSize === size ? '#f0fdf4' : '#fff',
                      color: pageSize === size ? '#059669' : '#374151',
                      cursor: 'pointer', fontFamily: 'Cairo,sans-serif', fontSize: 12, fontWeight: 700,
                      transition: 'all .15s',
                    }}>{size}</button>
                  ))}
                </div>
              ...
              <div style={{ marginBottom: 18 }}>
                <label style={fieldLabel}>🔄 اتجاه الورقة</label>
                <div style={{ display: 'flex', gap: 12 }}>
                  <button onClick={() => setOrientation('portrait')} style={{
                    flex: 1, padding: '14px 10px', border: orientation === 'portrait' ? '2px solid #10b981' : '1.5px solid #d1d5db',
                    borderRadius: 10, background: orientation === 'portrait' ? '#f0fdf4' : '#fff',
                    cursor: 'pointer', fontFamily: 'Cairo,sans-serif', transition: 'all .15s',
                  }}>
                    <div style={{ fontSize: 28, marginBottom: 4 }}>📄</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: orientation === 'portrait' ? '#059669' : '#6b7280' }}>طولي (Portrait)</div>
                  </button>
                  <button onClick={() => setOrientation('landscape')} style={{
                    flex: 1, padding: '14px 10px', border: orientation === 'landscape' ? '2px solid #10b981' : '1.5px solid #d1d5db',
                    borderRadius: 10, background: orientation === 'landscape' ? '#f0fdf4' : '#fff',
                    cursor: 'pointer', fontFamily: 'Cairo,sans-serif', transition: 'all .15s',
                  }}>
                    <div style={{ fontSize: 28, marginBottom: 4, transform: 'rotate(90deg)', display: 'inline-block' }}>📄</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: orientation === 'landscape' ? '#059669' : '#6b7280' }}>عرضي (Landscape)</div>
                  </button>
                </div>
              </div>

              <div style={{ marginBottom: 18 }}>
                <label style={fieldLabel}>📏 الهوامش (سم)</label>
                <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                  {[
                    { key: 'normal', label: 'عادي' },
                    { key: 'narrow', label: 'ضيق' },
                    { key: 'wide', label: 'واسع' },
                    { key: 'custom', label: 'مخصص' },
                  ].map(p => (
                    <button key={p.key} onClick={() => applyMarginPreset(p.key)} style={{
                      flex: 1, padding: '6px 4px', border: margins === p.key ? '2px solid #10b981' : '1.5px solid #d1d5db',
                      borderRadius: 7, background: margins === p.key ? '#f0fdf4' : '#fff',
                      color: margins === p.key ? '#059669' : '#374151',
                      cursor: 'pointer', fontFamily: 'Cairo,sans-serif', fontSize: 11, fontWeight: 700,
                    }}>{p.label}</button>
                  ))}
                </div>
                {margins === 'custom' && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {[
                      { label: 'أعلى', val: marginTop, set: setMarginTop },
                      { label: 'أسفل', val: marginBottom, set: setMarginBottom },
                      { label: 'يسار', val: marginLeft, set: setMarginLeft },
                      { label: 'يمين', val: marginRight, set: setMarginRight },
                    ].map(f => (
                      <div key={f.label}>
                        <label style={{ ...fieldLabel, fontSize: 11 }}>{f.label}</label>
                        <input type="number" min="0" max="5" step="0.1" value={f.val}
                          onChange={e => f.set(+e.target.value)}
                          style={{ width: '100%', padding: '6px 8px', border: '1.5px solid #d1fae5', borderRadius: 7, fontFamily: 'Cairo,sans-serif', outline: 'none', textAlign: 'center' }} />
                      </div>
                    ))}
                  </div>
                )}
          ...
          {activeTab === 'content' && (
            <>
              <div style={{ marginBottom: 18 }}>
                <label style={fieldLabel}>👁️ عناصر الصفحة</label>
                <div style={{ background: '#f9fafb', borderRadius: 10, padding: '12px 14px', border: '1px solid #e5e7eb' }}>
                  <Toggle value={showHeader} onChange={setShowHeader} label="🏢 رأس الصفحة (هيدر NileMix)" />
                  <Toggle value={showFooter} onChange={setShowFooter} label="📅 تذييل الصفحة (تاريخ + اسم)" />
                  <Toggle value={showSignature} onChange={setShowSignature} label="✍️ منطقة التوقيع" />
                  <Toggle value={repeatHeader} onChange={setRepeatHeader} label="🔁 تكرار رأس الجدول في كل صفحة" />
                </div>
              </div>

              <div style={{ marginBottom: 18 }}>
                <label style={fieldLabel}>🎨 وضع الألوان</label>
                <div style={{ display: 'flex', gap: 8 }}>
                  {[
                    { key: 'color', label: '🌈 ألوان', desc: 'طباعة ملونة كاملة' },
                    { key: 'grayscale', label: '⬜ رمادي', desc: 'تدرجات الرمادي' },
                    { key: 'blackwhite', label: '⬛ أبيض وأسود', desc: 'أبيض وأسود فقط' },
                  ].map(m => (
                    <button key={m.key} onClick={() => setColorMode(m.key)} style={{
                      flex: 1, padding: '10px 6px', border: colorMode === m.key ? '2px solid #10b981' : '1.5px solid #d1d5db',
                      borderRadius: 9, background: colorMode === m.key ? '#f0fdf4' : '#fff',
                      cursor: 'pointer', fontFamily: 'Cairo,sans-serif', transition: 'all .15s',
                    }}>
                      <div style={{ fontSize: 18, marginBottom: 3 }}>{m.label.split(' ')[0]}</div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: colorMode === m.key ? '#059669' : '#374151' }}>{m.label.split(' ')[1]}</div>
                      <div style={{ fontSize: 9, color: '#9ca3af', marginTop: 2 }}>{m.desc}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={fieldLabel}>🔡 حجم الخط الأساسي: <span style={{ color: '#10b981' }}>{fontSize}px</span></label>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={fieldLabel}>📐 ارتفاع السطر: <span style={{ color: '#10b981' }}>{lineHeight}</span></label>
                <input type="range" min={1.2} max={2.5} step={0.1} value={lineHeight} onChange={e => setLineHeight(+e.target.value)}
                  style={{ width: '100%', accentColor: '#10b981' }} />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: '#9ca3af' }}>
                  <span>مضغوط 1.2</span><span>واسع 2.5</span>
                </div>
              </div>
            </>
          )}
          ...
          {activeTab === 'advanced' && (
            <>
              <div style={{ background: '#f0fdf4', border: '1.5px solid #a7f3d0', borderRadius: 10, padding: 14, marginBottom: 16 }}>
                <div style={{ fontSize: 13, fontWeight: 800, color: '#065f46', marginBottom: 10 }}>📋 ملخص الإعدادات الحالية</div>
                {[
                  { label: 'حجم الورقة', val: pageSize },
                  { label: 'الاتجاه', val: orientation === 'portrait' ? 'طولي' : 'عرضي' },
                  { label: 'الهوامش', val: margins === 'custom' ? `مخصص (${marginTop}/${marginBottom}/${marginLeft}/${marginRight} سم)` : margins === 'normal' ? 'عادي' : margins === 'narrow' ? 'ضيق' : 'واسع' },
                  { label: 'الألوان', val: colorMode === 'color' ? 'ملون' : colorMode === 'grayscale' ? 'رمادي' : 'أبيض وأسود' },
                  { label: 'حجم الخط', val: `${fontSize}px` },
                  { label: 'ارتفاع السطر', val: lineHeight },
                  { label: 'الهيدر', val: showHeader ? 'مفعل' : 'مخفي' },
                  { label: 'الفوتر', val: showFooter ? 'مفعل' : 'مخفي' },
                  { label: 'التوقيع', val: showSignature ? 'مفعل' : 'مخفي' },
                  { label: 'تكرار رأس الجدول', val: repeatHeader ? 'نعم' : 'لا' },
                ].map(r => (
                  <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#374151', marginBottom: 4, direction: 'rtl' }}>
                    <span style={{ fontWeight: 600, color: '#6b7280' }}>{r.label}:</span>
                    <span style={{ fontWeight: 700, color: '#059669' }}>{r.val}</span>
                  </div>
                ))}
              </div>

              <div style={{ background: '#fef9c3', border: '1px solid #fde68a', borderRadius: 8, padding: 12, fontSize: 12, color: '#78350f', direction: 'rtl', lineHeight: 1.8 }}>
                <div style={{ fontWeight: 800, marginBottom: 4 }}>💡 تلميحات مفيدة:</div>
                <div>• لحفظ PDF: اضغط "تصدير PDF" ثم اختر "حفظ باسم PDF" في مربع الطباعة</div>
                <div>• للطباعة المباشرة: اضغط "طباعة مباشرة" وسيُرسل للطابعة فوراً</div>
                <div>• حجم A3 مناسب للجداول العريضة</div>
                <div>• الوضع الرمادي يوفر الحبر في الطابعات الملونة</div>
              </div>
            </>
          )}
        </div>

        <div style={{ padding: '16px 24px', borderTop: '1px solid #e5e7eb', display: 'flex', gap: 10, flexDirection: 'column' }}>
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              onClick={() => { onDirectPrint(printSettings); onClose(); }}
              style={{
                flex: 1, padding: '12px 10px',
                background: 'linear-gradient(135deg,#1d4ed8,#1e40af)',
                color: '#fff', border: 'none', borderRadius: 10,
                fontFamily: 'Cairo,sans-serif', fontSize: 13, fontWeight: 800,
                cursor: 'pointer', boxShadow: '0 4px 14px rgba(29,78,216,.3)',
                transition: 'all .15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
              }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(29,78,216,.4)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(29,78,216,.3)'; }}>
              🖨️ طباعة مباشرة
            </button>

            <button
              onClick={() => { onPrint(printSettings); onClose(); }}
              style={{
                flex: 1, padding: '12px 10px',
                background: 'linear-gradient(135deg,#dc2626,#b91c1c)',
                color: '#fff', border: 'none', borderRadius: 10,
                fontFamily: 'Cairo,sans-serif', fontSize: 13, fontWeight: 800,
                cursor: 'pointer', boxShadow: '0 4px 14px rgba(220,38,38,.3)',
                transition: 'all .15s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
              }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 20px rgba(220,38,38,.4)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(220,38,38,.3)'; }}>
              📄 تصدير PDF
            </button>
          </div>

          <button className="tbl-cancel" onClick={onClose} style={{ width: '100%' }}>إلغاء</button>
        </div>
      </div>
    </div>
  );
...
export default function DocumentEditor() {
  ...
  // ─── معالجة الأرقام عند الإدخال في المحرر ─────────────────────────────
  const handleInputAndConvertNumbers = useCallback((targetNode) => {
    if (targetNode.nodeType === Node.TEXT_NODE) {
      if (targetNode.textContent && /[0-9]/.test(targetNode.textContent)) {
        const converted = convertToArabicNumbers(targetNode.textContent);
        if (converted !== targetNode.textContent) {
          targetNode.textContent = converted;
        }
      }
    } else if (targetNode.nodeType === Node.ELEMENT_NODE) {
      targetNode.childNodes.forEach(handleInputAndConvertNumbers);
    }
  }, []);

  // ─── مراقبة أحداث الإدخال في المحرر الرئيسي ───────────────────────────
  useEffect(() => {
    ...
    const onInput = (e) => {
      ...
      if (target) {
        handleInputAndConvertNumbers(target);
      }
      updateCounts();
    };
    ...
    const onBeforeInput = (e) => {
      ...
      if (data && /[0-9]/.test(data)) {
        e.preventDefault();
        const converted = convertToArabicNumbers(data);
  ...
  const saveDocument = useCallback(async (silent = false) => {
    const authToken = token || localStorage.getItem('token');
    if (!authToken) {
      if (!silent) alert('الرجاء تسجيل الدخول أولاً');
      navigate('/login');
      return;
    }
    ...
    if (!content || content === '<br>' || content.trim() === '') {
      if (!silent) alert('المستند فارغ، لا يمكن الحفظ');
      return;
    }
    try {
      ...
      if (res.status === 401) {
        if (!silent) alert('انتهت صلاحية الجلسة، يرجى تسجيل الدخول مرة أخرى');
        logout();
        navigate('/login');
        return;
      }
  ...
  }, [title, currentDocId, token, logout, navigate]);
  ...
  useEffect(() => {
    autoSaveTimer.current = setInterval(() => {
      ...
      if (c && c.length > 5 && isAuthenticated) saveDocument(true);
    }, 30000);
    return () => clearInterval(autoSaveTimer.current);
  }, [saveDocument, isAuthenticated]);
  ...
  const makeEditorCell = useCallback((tag, colW = 100) => {
    ...
    // معالجة الأرقام في الخلية
    const handleCellBeforeInput = (e) => {
      const data = e.data;
      if (data && /[0-9]/.test(data)) {
        e.preventDefault();
        document.execCommand('insertText', false, convertToArabicNumbers(data));
      }
    };
    
    const handleCellInput = (e) => {
      handleInputAndConvertNumbers(e.target);
    };
    
    el.addEventListener('beforeinput', handleCellBeforeInput);
  ...
  const buildPrintHTML = (settings = {}) => {
    ...
    const colorFilter = colorMode === 'grayscale'
      ? 'filter: grayscale(100%);'
      : colorMode === 'blackwhite'
      ? 'filter: grayscale(100%) contrast(200%);'
      : '';

    const orientationCSS = `@page { size: ${pageSize} ${orientation}; margin: ${marginTop}cm ${marginRight}cm ${marginBottom}cm ${marginLeft}cm; }`;
...
<style>
  ...
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  html, body { margin: 0; padding: 0; font-family: 'Cairo', sans-serif; font-size: ${fontSize}px; line-height: ${lineHeight}; color: #111; background: #fff; ${colorFilter} }
  .print-page { width: 100%; margin: 0 auto; padding: 0.3cm; }
  .print-hdr { display: ${showHeader ? 'flex' : 'none'}; align-items: flex-start; justify-content: space-between; border-bottom: 3px solid #10b981; padding-bottom: 10px; margin-bottom: 20px; }
  .print-hdr-text-block { display: flex; flex-direction: column; line-height: 1; }
  .print-hdr-brand { color: #10b981; font-size: 22px; font-weight: 900; margin: 0; padding: 0; line-height: 1.1; }
  .print-hdr-sub { color: #0f766e; font-weight: 700; font-size: 12px; margin: 0; padding: 0; line-height: 1.2; }
  .print-hdr-logo { width: 64px; height: 64px; object-fit: contain; flex-shrink: 0; }
  .print-hdr-logo-fallback { width: 64px; height: 64px; background: linear-gradient(135deg, #10b981, #0f766e); border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 28px; color: white; font-weight: 900; flex-shrink: 0; font-family: 'Cairo', sans-serif; }
  .doc-title { text-align: center; color: #065f46; font-size: ${Math.round(fontSize * 1.3)}px; font-weight: 900; margin: 15px 0 20px; padding-bottom: 8px; border-bottom: 2px dashed #a7f3d0; }
  .content-body { margin: 20px 0; font-size: ${fontSize}px; line-height: ${lineHeight}; }
  .content-body p { margin: 10px 0; word-break: break-word; }
  .content-body h1 { font-size: ${Math.round(fontSize * 1.3)}px; font-weight: 900; color: #065f46; margin: 15px 0 10px; }
  .content-body h2 { font-size: ${Math.round(fontSize * 1.15)}px; font-weight: 800; color: #0f766e; margin: 12px 0 8px; }
  .content-body h3 { font-size: ${fontSize}px; font-weight: 700; color: #047857; margin: 10px 0 6px; }
  table { border-collapse: collapse !important; direction: rtl !important; table-layout: auto !important; width: 100% !important; margin: 15px 0 !important; page-break-inside: avoid !important; }
  th, td { border: 1.5px solid #000 !important; text-align: center !important; vertical-align: middle !important; word-break: break-word !important; overflow-wrap: break-word !important; direction: rtl !important; padding: 8px 12px !important; font-family: 'Cairo', sans-serif !important; font-size: ${Math.round(fontSize * 0.9)}px !important; }
  th { font-weight: 700 !important; background: #f0f0f0 !important; color: #000 !important; }
  thead { display: ${repeatHeader ? 'table-header-group' : 'table-header-group'} !important; }
  tbody { display: table-row-group !important; }
  tr { page-break-inside: avoid !important; page-break-after: auto !important; }
  td { page-break-inside: avoid !important; }
  td table { width: 100% !important; height: 100% !important; border: none !important; margin: 0 !important; }
  td table td { border: 1px dashed #059669 !important; font-size: ${Math.round(fontSize * 0.85)}px !important; padding: 4px 6px !important; }
  .print-tbl-wrap { display: block; margin: 15px 0; overflow-x: auto; page-break-inside: avoid; }
  .print-footer { display: ${showFooter ? 'block' : 'none'}; margin-top: 40px; padding-top: 14px; border-top: 2px solid #d1fae5; }
  .print-footer-top { display: flex; justify-content: space-between; align-items: center; font-size: 10px; color: #555; margin-bottom: 20px; }
  .print-footer-brand { color: #10b981; font-weight: 800; }
  .print-signature-area { display: ${showSignature ? 'flex' : 'none'}; flex-direction: column; align-items: flex-start; gap: 0; margin-top: 10px; font-family: 'Cairo', sans-serif; direction: rtl; }
  .print-signature-label { font-size: 13px; font-weight: 700; color: #111; margin-bottom: 2px; }
  .print-signature-title { font-size: 12px; font-weight: 600; color: #374151; margin-bottom: 2px; }
  .print-signature-line { font-size: 13px; color: #374151; letter-spacing: 2px; margin-top: 2px; font-family: monospace; }
  ul, ol { margin: 10px 20px; padding-right: 20px; }
  li { margin: 5px 0; }
  h1, h2, h3, h4, h5, h6, p, li, blockquote { page-break-inside: avoid; }
...
<div class="print-page">
  <div class="print-hdr">
    ...
    <img src="${process.env.REACT_APP_LOGO_URL || '/logo.png'}" class="print-hdr-logo" alt="NileMix Logo"
...
<div class="no-print" style="position:fixed;bottom:20px;right:20px;z-index:9999;display:flex;gap:10px;">
  ...
  <button onclick="window.close()" style="background:#374151;color:white;border:none;padding:12px 20px;border-radius:10px;cursor:pointer;font-size:13px;font-family:Cairo,sans-serif;font-weight:700;">
    إغلاق
  </button>
</div>
</body>
  ...
  const handleDirectPrint = (settings = {}) => {
    const html = buildPrintHTML(settings);
  ...
  if (!isAuthenticated && !token) {
    return <div style={{ textAlign: 'center', padding: '50px', fontSize: '18px' }}>جاري التحقق من تسجيل الدخول...</div>;
  }
...
.btn-print-direct{background:linear-gradient(135deg,#1d4ed8,#1e40af);color:#fff;box-shadow:0 3px 10px rgba(29,78,216,.25)}
.btn-print-direct:hover{box-shadow:0 5px 16px rgba(29,78,216,.35);transform:translateY(-1px)}
.btn-print-pdf{background:linear-gradient(135deg,#dc2626,#b91c1c);color:#fff;box-shadow:0 3px 10px rgba(220,38,38,.25)}
.btn-print-pdf:hover{box-shadow:0 5px 16px rgba(220,38,38,.35);transform:translateY(-1px)}
      ...
      <div className="de-page">
        <div className="de-header">
          ...
          <div className="de-actions">
            ...
            <div className="hdr-sep" />
            <button
              className="de-btn btn-print-direct"
              onClick={() => handleDirectPrint({ orientation: 'portrait', pageSize: 'A4', showHeader: true, showFooter: true, showSignature: true, colorMode: 'color', fontSize: 14, lineHeight: 1.7, marginTop: 1.5, marginBottom: 1.5, marginLeft: 1.5, marginRight: 1.5, repeatHeader: true })}
              title="طباعة مباشرة بالإعدادات الافتراضية"
            >
              🖨️ طباعة
            </button>
            <button
              className="de-btn btn-print-pdf"
              onClick={() => setShowPrintOptions(true)}
              title="خيارات الطباعة والتصدير PDF"
            >
              📄 PDF / إعدادات
            </button>
        ...
        {showSavedDocs && <SavedDocsDialog onLoad={loadDocument} onClose={() => setShowSavedDocs(false)} token={token} />}
