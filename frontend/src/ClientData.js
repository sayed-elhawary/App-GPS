...
// ============ إعدادات البيئة ============
const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://192.168.127.141:5000';
const DEFAULT_THEME = process.env.REACT_APP_DEFAULT_THEME || 'dark';
...
export default function ClientData() {
  ...
  // تحميل الثيم المحفوظ
  useEffect(() => {
    const savedTheme = localStorage.getItem('clientDataTheme') || DEFAULT_THEME;
    setTheme(savedTheme);
  }, []);
  ...
  const fetchAllEmployees = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/client-data/employees`);
...
1 | 1 | هانى محمد حسن محمد على العدل | 42150432`);
  ...
  const handleSubmit = async () => {
    ...
    setSuccess('');
    try {
      const res = await axios.post(`${API_BASE_URL}/api/client-data/bulk`, {
  ...
  // دالة إضافة موظف جديد يدوياً
  const handleAddEmployee = async () => {
    if (!newEmployeeCode.trim() || !newEmployeeName.trim()) {
      setError('يجب إدخال كود الموظف واسم الموظف');
      return;
    }
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/api/client-data/employees`, {
      ...
      setError(err.response?.data?.message || 'فشل في إضافة الموظف');
    } finally {
      setAdding(false);
    }
  };
  ...
  const handleDeleteAll = async () => {
    ...
    setSuccess('');
    try {
      await axios.delete(`${API_BASE_URL}/api/client-data/employees`);
      ...
      <style>{`
        ...
        * {
        ...
        .modal {
          ...
          width: 420px;
          padding: 30px;
          box-shadow: 0 20px 40px -10px ${themeStyles.shadow};
        }
          ...
          <div className="header">
            <div className="header-left">
              ...
              <div>
                ...
                <div className="subtitle">رفع • بحث • إضافة • إدارة بيانات الموظفين</div>
              </div>
            </div>
                      ...
                      <td colSpan="3">
                        <div className="empty-state">
                          ...
                          <div style={{ fontSize: '12px', marginTop: '8px' }}>
                            قم برفع ملف Excel أو أضف موظف يدوياً
                          </div>
                        </div>
                      </td>
                    </tr>
      ...
      {showAddModal && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            <input
              type="text"
              placeholder="كود الموظف (مثال: 25)"
            ...
            <div className="modal-buttons">
              ...
              <button 
                ...
                disabled={adding}
              >
                {adding ? 'جاري الإضافة...' : 'حفظ الموظف'}
              </button>
            </div>
          </div>
        </div>
