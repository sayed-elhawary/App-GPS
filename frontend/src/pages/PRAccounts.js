...
const PRAccounts = () => {
  ...
  // ─── بناء HTML التقرير المشترك ────────────────────────────────────────────
  const buildTotalReportHTML = () => {
  ...
  const buildInvoiceReportHTML = (invoice) => {
    const invoiceExpenses = expenses.filter(exp => exp.invoiceId === invoice._id);
  ...
  // ─── تصدير التقرير الإجمالي PDF ──────────────────────────────────────────
  const exportTotalReport = async () => {
    setLoading(true);
    try {
      const reportElement = document.createElement('div');
      reportElement.style.cssText = `
        position: fixed; top: -99999px; left: -99999px; width: 1200px;
        background: white; padding: 40px; direction: rtl;
        font-family: 'Cairo', 'Arial', sans-serif; color: #000000; z-index: -9999;
      `;
      reportElement.innerHTML = buildTotalReportHTML();
      document.body.appendChild(reportElement);
      await new Promise(resolve => setTimeout(resolve, 300));
      const canvas = await html2canvas(reportElement, { scale: 3, backgroundColor: '#ffffff', logging: false, useCORS: true });
  ...
  // ─── طباعة التقرير الإجمالي مباشرة ──────────────────────────────────────
  const printTotalReport = () => {
    const currentDate = new Date().toLocaleDateString('ar-EG');
...
<head>
<meta charset="UTF-8">
<title>تقرير الحسابات - NileMix</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap" rel="stylesheet">
<style>
  @page { size: A4 landscape; margin: 1cm; }
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  body { font-family: 'Cairo', Arial, sans-serif; direction: rtl; background: #fff; color: #000; margin: 0; padding: 20px; }
  table { width: 100%; border-collapse: collapse; margin: 15px 0; }
  th, td { border: 1px solid #000; padding: 10px 12px; text-align: center; font-size: 13px; }
...
<body>
${buildTotalReportHTML()}
<div class="no-print" style="position:fixed;bottom:20px;right:20px;display:flex;gap:10px;">
  <button onclick="window.print()" style="background:#10b981;color:white;border:none;padding:12px 24px;border-radius:10px;cursor:pointer;font-size:14px;font-family:Cairo,sans-serif;font-weight:700;">🖨️ طباعة</button>
  <button onclick="window.close()" style="background:#374151;color:white;border:none;padding:12px 20px;border-radius:10px;cursor:pointer;font-size:13px;font-family:Cairo,sans-serif;font-weight:700;">إغلاق</button>
</div>
</body></html>`);
    win.document.close();
    win.onload = () => { setTimeout(() => { win.focus(); win.print(); }, 600); };
    setTimeout(() => { try { if (!win.closed) { win.focus(); win.print(); } } catch(e){} }, 1500);
  };
  ...
  const exportInvoiceReport = async (invoice) => {
    ...
    try {
      const reportElement = document.createElement('div');
      reportElement.style.cssText = `
        position: fixed; top: -99999px; left: -99999px; width: 1000px;
        background: white; padding: 35px; direction: rtl;
        font-family: 'Cairo', 'Arial', sans-serif; color: #000000; z-index: -9999;
      `;
      reportElement.innerHTML = buildInvoiceReportHTML(invoice);
      document.body.appendChild(reportElement);
      await new Promise(resolve => setTimeout(resolve, 300));
      const canvas = await html2canvas(reportElement, { scale: 3, backgroundColor: '#ffffff', logging: false, useCORS: true });
      const imgData = canvas.toDataURL('image/png');
  ...
  // ─── طباعة فاتورة مباشرة ─────────────────────────────────────────────────
  const printInvoiceReport = (invoice) => {
...
<head>
<meta charset="UTF-8">
<title>فاتورة - ${invoice.description}</title>
<link href="https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap" rel="stylesheet">
<style>
...
<body>
${buildInvoiceReportHTML(invoice)}
<div class="no-print" style="position:fixed;bottom:20px;right:20px;display:flex;gap:10px;">
  <button onclick="window.print()" style="background:#10b981;color:white;border:none;padding:12px 24px;border-radius:10px;cursor:pointer;font-size:14px;font-family:Cairo,sans-serif;font-weight:700;">🖨️ طباعة</button>
  <button onclick="window.close()" style="background:#374151;color:white;border:none;padding:12px 20px;border-radius:10px;cursor:pointer;font-size:13px;font-family:Cairo,sans-serif;font-weight:700;">إغلاق</button>
</div>
    ...
    <>
      <style>{`
        ...
        * {
        ...
        .reset-btn, .export-btn, .print-btn {
          padding: 12px 20px;
          border: none;
          border-radius: 12px;
          font-weight: 700;
        ...
        .pdf-action-btn { color: #10b981; }
        .pdf-action-btn:hover { background: rgba(16, 185, 129, 0.1); transform: scale(1.1); }
        .print-action-btn { color: #1d4ed8; }
        .print-action-btn:hover { background: rgba(29, 78, 216, 0.1); transform: scale(1.1); }
        ...
        .modal-close { background: #ef4444; color: white; border: none; padding: 6px 18px; border-radius: 8px; cursor: pointer; font-weight: 700; font-family: 'Cairo', sans-serif; }
        .modal-export { background: #10b981; color: white; border: none; padding: 6px 18px; border-radius: 8px; cursor: pointer; font-weight: 700; font-family: 'Cairo', sans-serif; }
        .modal-print { background: #1d4ed8; color: white; border: none; padding: 6px 18px; border-radius: 8px; cursor: pointer; font-weight: 700; font-family: 'Cairo', sans-serif; }
        ...
        <div className="pr-container">
          ...
          <div className="search-card">
            <div className="search-grid">
              ...
              {/* ── أزرار: إعادة تعيين + PDF + طباعة ── */}
              <div className="search-group" style={{ display: 'flex', flexDirection: 'row', gap: '10px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                ...
                <button onClick={printTotalReport} disabled={loading} className="print-btn">
                  🖨️ طباعة
                </button>
              </div>
            </div>
                    ...
                    return (
                      <tr key={inv._id}>
                        ...
                        <td>
                          ...
                          {/* تصدير PDF */}
                          <button onClick={() => exportInvoiceReport(inv)} disabled={loading} className="action-btn pdf-action-btn" title="تصدير PDF">📄</button>
                          {/* طباعة مباشرة */}
                          <button onClick={() => printInvoiceReport(inv)} className="action-btn print-action-btn" title="طباعة مباشرة">🖨️</button>
                          {canEdit() && (
                            <>
            ...
            <div className="modal-content" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {/* زرار طباعة مباشرة */}
                  <button onClick={() => printInvoiceReport(selectedInvoice)} className="modal-print">
                    🖨️ طباعة
                  </button>
