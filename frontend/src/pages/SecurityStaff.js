...
export default function SecurityStaff() {
  ...
  // جلب اسم الموظف تلقائياً حسب الكود
  ...
  const handleExportPDF = async () => {
    ...
    setPdfLoading(true);
    try {
      await exportToPDF(filteredStaff);
    } catch (err) {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html dir="rtl">
        <head>
          <title>تقرير موظفي الأمن - NileMix</title>
          <style>
        ...
        <body>
          <div class="header">
            <h1>تقرير بيانات موظفي الأمن</h1>
            <p>شركة النيل للخرسانة الجاهزة - NileMix</p>
            <p>تاريخ الطباعة: ${today}</p>
            <p>إجمالي الموظفين: ${filteredStaff.length}</p>
          </div>
          <table>
            <thead>
              <tr>
      ...
      <div style={{ background: themeStyles.cardBg, borderRadius: '24px', padding: '35px', boxShadow: `0 20px 40px -12px ${themeStyles.shadow}` }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <button 
              onClick={handlePrint} 
              disabled={printLoading || filteredStaff.length === 0} 
              style={{ background: '#10b981', color: 'white', border: 'none', padding: '10px 22px', borderRadius: '14px', fontWeight: '800', cursor: printLoading || filteredStaff.length === 0 ? 'not-allowed' : 'pointer' }}
            >
            ...
            <button 
              ...
              disabled={pdfLoading || filteredStaff.length === 0} 
              style={{ background: themeStyles.primaryBtn, color: 'white', border: 'none', padding: '10px 22px', borderRadius: '14px', fontWeight: '800', cursor: pdfLoading || filteredStaff.length === 0 ? 'not-allowed' : 'pointer' }}
            >
              {pdfLoading ? '⏳ جاري...' : '📄 PDF'}
            </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '18px', marginBottom: '30px' }}>
          <div style={{ background: themeStyles.statBg, border: `1px solid ${themeStyles.statBorder}`, borderRadius: '20px', padding: '20px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '2rem', fontWeight: '900', color: themeStyles.statValue }}>{filteredStaff.length}</div>
            <div style={{ fontSize: '0.8rem', color: themeStyles.text3 }}>إجمالي الموظفين (المعروض)</div>
          </div>
          <div style={{ background: themeStyles.statBg, border: `1px solid ${themeStyles.statBorder}`, borderRadius: '20px', padding: '20px', textAlign: 'center' }}>
            <div style={{ fontSize: '2rem' }}>📍</div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="7" style={{ textAlign: 'center', padding: '60px', color: themeStyles.text3 }}>📭 لا توجد بيانات مطابقة للبحث</td></tr>
              ) : (
                filteredStaff.map((person, idx) => (
                  <tr key={person._id} style={{ background: idx % 2 === 0 ? 'transparent' : themeStyles.tableRowHover }}>
        ...
        {!loading && filteredStaff.length > 0 && (
          <div style={{ marginTop: '20px', textAlign: 'center', color: themeStyles.text3, fontSize: '0.8rem' }}>
            إجمالي {filteredStaff.length} موظف معروض من أصل {staff.length}
          </div>
        )}
      </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: themeStyles.modalBg, borderRadius: '24px', width: '100%', maxWidth: '550px', maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <div style={{ padding: '20px 28px', background: themeStyles.tableHeaderBg, borderTop: `1px solid ${themeStyles.cardBorder}`, display: 'flex', gap: '15px', justifyContent: 'flex-end' }}>
              ...
              <button onClick={handleSave} disabled={loading || fetchingEmployee} style={{ background: themeStyles.primaryBtn, color: 'white', border: 'none', padding: '10px 22px', borderRadius: '14px', cursor: loading || fetchingEmployee ? 'not-allowed' : 'pointer', opacity: loading || fetchingEmployee ? 0.6 : 1 }}>
                {loading ? '⏳ جاري...' : (editingStaff ? 'حفظ التعديل' : 'إضافة الموظف')}
              </button>
            </div>
          </div>
        </div>
