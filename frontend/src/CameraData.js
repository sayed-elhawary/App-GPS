...
export default function CameraData() {
 ...
 const handleSave = async (e) => {
  ...
  const dataToSave = {
    ...
    cameraType_mm: parseInt(formData.cameraType_mm) || 0,
    cameraType_4m: parseInt(formData.cameraType_4m) || 0,
    cameraType_5m: parseInt(formData.cameraType_5m) || 0,
    cameraType_6m: parseInt(formData.cameraType_6m) || 0,
    numCameras: totalCameras,
    stabilizer: formData.stabilizer.trim(),
    usb: formData.usb.trim(),
    inverter: formData.inverter.trim(),
    routerType: formData.routerType.trim(),
    screenType: formData.screenType.trim(),
    switchVoltage: formData.switchVoltage.trim(),
    switchPorts: formData.switchPorts.trim(),
    simNumber: formData.simNumber.trim(),
    simSerial: formData.simSerial.trim(),
  ...
  if (!dataToSave.simNumber) {
    setError('رقم الشريحة مطلوب');
    return;
  }

  console.log('📤 جاري إرسال البيانات:', dataToSave);   // للتصحيح
  ...
  try {
    ...
    if (editingCamera) {
      // === تعديل ===
      console.log(`🔄 جاري تعديل الكاميرا ID: ${editingCamera._id}`);
      response = await axios.put(
        `${API_URL}/api/cameras/${editingCamera._id}`, 
        dataToSave
      );
      console.log('✅ تم التعديل بنجاح:', response.data);
      setSuccess('✅ تم تعديل بيانات الكاميرا بنجاح');
    } else {
      // === إضافة جديدة ===
      console.log('➕ جاري إضافة كاميرا جديدة');
      response = await axios.post(`${API_URL}/api/cameras`, dataToSave);
      console.log('✅ تمت الإضافة بنجاح:', response.data);
      setSuccess('✅ تم إضافة الكاميرا الجديدة بنجاح');
    }
    ...
    console.error('❌ خطأ كامل أثناء الحفظ:', err);
...
const handleDelete = async (id) => {
  if (!id) {
    setError('معرف الكاميرا غير صحيح');
    setDeleteConfirm(null);
    return;
  }

  try {
    console.log(`🗑️ جاري حذف الكاميرا ID: ${id}`);
    
    const response = await axios.delete(`${API_URL}/api/cameras/${id}`);
    
    console.log('✅ تم الحذف بنجاح:', response.data);
    setSuccess('✅ تم حذف البيانات بنجاح');
    setDeleteConfirm(null);
    
    await fetchAllCameras();   // تحديث الجدول
  } catch (err) {
    console.error('❌ خطأ أثناء الحذف:', err.response?.data || err.message);
    
    const errorMsg = err.response?.data?.message || 'فشل في حذف البيانات';
    setError(`❌ ${errorMsg}`);
    setDeleteConfirm(null);
  }
};
